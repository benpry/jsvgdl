...
$(document).ready(function () {
	...
	var forfeit_text =
		$(`<p id="forfeit-text">If you want to forfeit the entire game and try a new one, press "Forfeit".
	...
	var retry_text = $(
		'<p id="retry-text">If you get stuck, you can press "Retry" to reset this level.</p>',
	);
	var retry_button = $('<button id="retry">Retry</button>');
	retry_container.append(retry_text);
	retry_container.append(retry_button);

	var overtime_container = $(
		'<div id="overtime" class="Flex-Container"></div>',
	);
	var overtime_text =
		$(`<p id="overtime_text">You’ve now been playing for 25 minutes.
	...
	if (show_score) {
		var score_container = $(
			'<h2 id="score">Score: <span id="score-value">0</span></h2>',
		);
	...
	var begin_game = function () {
		...
		// }
		window.setTimeout(function () {
			if (!ended) $("body").append(forfeit_div);
		}, forfeit_delay);
	};
