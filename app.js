...
function validate_exp(req, res, next) {
	...
	if (
		...
		Experiment.validate(req.session.experiment, val_id)
	) {
		next();
	} else {
...
app.get(
	"/play/:game_name/level/:level/desc/:desc",
	require_login,
	function (req, res) {
...
app.get("/experiment/:exp_id", validate_exp, function (req, res, next) {
	...
	if (!current_exp) {
		...
		res.render("overtime_cont", {
			games: Experiment.remaining_games(current_exp),
			exp_id: data.exp_id,
		});
